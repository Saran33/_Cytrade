...
cpdef add_hist_features(str ticker, object sec, str interval='1h', unsigned long trading_periods=365, unsigned long market_hours=24,
                        unsigned long vol_window=60, float test_size=0.2):
    ...
    cdef dict pair_expirations
    cdef object pair_last_fris_df
    ...
    assert isinstance(pair_last_fris_df, pd.DataFrame)
    ...
    # cdef np.ndarray[np.uint8_t, cast=True] keep_columns, tminus_mask
