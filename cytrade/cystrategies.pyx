...
class KellyML(CyStrategy):
    # def __init__(self):
        ...
        # self.mean = bt.indicators.MovingAverageSimple(self.pctChange, period=self.p.window)
        # self.std = bt.indicators.StdDev(self.pctChange, period=self.p.window)
        # for data in self.datas:
        #     self.data.kelly = single_kelly(self.data.sma[0], self.std[0])
    ...
    def next(self):
        ...
        # if self.data.sma[0]:
        #     kelly = ll_single_kelly(self.mean[0], self.std[0], bounds=np.array([0., 2.]))
        # else:
        #     kelly = 0
        for data in self.datas:
