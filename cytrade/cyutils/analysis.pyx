...
from libc.math cimport ceil, pow, log, sqrt
...
cdef np.ndarray[double] apply_log(np.ndarray[double] arr):
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef np.ndarray[double] apply_sqrt(np.ndarray[double] arr):
...
cdef class Security:
    ...
    def __cinit__(self, inp):
        ...
        if hasattr(self.df, 'name'):
            self.name = self.ticker = self.df.name
        else:
            raise Exception(
    ...
    cpdef np.ndarray[double] rolling_cum_returns(self, np.ndarray[double] returns, double starting_value=0):
        ...
        cdef np.ndarray[double, ndim=1] out = np.empty_like(returns)
        # cdef double[:] c_out = memoryview(out)
    ...
    cpdef void get_returns(self, returns='Price_Returns', price='close'):
        """
        ...
        shiftRets = np_shift(priceArray, 1)
        df[f'{returns}'] = priceArray / shiftRets - 1;

        df['Log_Returns'] = np.log(priceArray/shiftRets);

        df['Cumulative Returns'] = self.rolling_cum_returns(df[f'{returns}'].to_numpy())

    cpdef double get_geomean(self, returns, axis=None):
        cdef np.ndarray[double, ndim=1] rets = np.asarray(returns)
        valids = rets[~np.isnan(rets)]
        cum_ret = np.nanprod((1 + valids), axis=axis)
        cdef int periods = np.count_nonzero(valids)
        return pow(cum_ret, 1 / periods) - 1

    cpdef double get_cum_returns(self, price='close'):
        '''The final cumulative return for the series.'''
        df = self.df
        return (df[price].iloc[-1] / df[price].iloc[0])-1


    cpdef double annualized_return(self, returns, ann_factor=252, cum_rets=None):
        if len(returns) < 1:
            return np.nan
        if not cum_rets:
    ...
    cpdef cumulate_returns(self, x):
        return self.rolling_cum_returns(x).iloc[-1]

    def aggregate_returns(self, returns, convert_to) -> pd.Series:
        """
        ...
        return returns.groupby(grouping).apply(self.cumulate_returns)

    cpdef double max_drawdown(self, returns):
        ...
        cumulative[1:] = self.rolling_cum_returns(
            returns_array, starting_value=start)

        max_return = np.fmax.accumulate(cumulative, axis=0)

        return np.nanmin((cumulative - max_return) / max_return, axis=0)

    cpdef void vwap(self, h='high', l='low', c='close', v='volume', window=None):
        """
    ...
    cpdef vwp(self, df, price, volume):
        """
    ...
    cpdef vwap_close(self, window=1, price='close', volume='volume'):
        """
    ...
    cpdef tuple get_ann_factor(self, str interval='1d', unsigned long trading_periods=252, unsigned long market_hours=24):
        cdef double ann_factor
        cdef str t, p
        if interval == '1d':
    ...
    cpdef tuple get_vol(self, unsigned long window=21, str returns='Price_Returns', unsigned long trading_periods=252, str interval='1d',
                unsigned long market_hours=24):
        """
        1 month window = 21
        3 month window = 63
    ...
    cpdef tuple YangZhang_estimator(self, unsigned long window=6, unsigned long trading_periods=252, bint clean=True, str interval='daily', unsigned long market_hours=24):
        """
        ...
        cdef np.ndarray[np.float64_t, ndim=1] o, h, l, c 
        o = df['open'].to_numpy()
        h = df['high'].to_numpy()
        l = df['low'].to_numpy()
        c = df['close'].to_numpy()
        ...
        cdef np.ndarray[np.float64_t, ndim=1] ho = np.divide(h, o)
        cdef np.ndarray[np.float64_t, ndim=1] log_ho = apply_log(ho)

        # log_lo = (df['low'] / df['open']).apply(np.log)
        cdef np.ndarray[np.float64_t, ndim=1] lo = np.divide(l, o)
        cdef np.ndarray[np.float64_t, ndim=1] log_lo = apply_log(lo)
        
        # log_co = (df['close'] / df['open']).apply(np.log)
        cdef np.ndarray[np.float64_t, ndim=1] co = np.divide(c, o)
        cdef np.ndarray[np.float64_t, ndim=1] log_co = apply_log(co)

        # log_oc = (df['open'] / df['close'].shift(1)).apply(np.log)
        cdef np.ndarray[np.float64_t, ndim=1] oc = np.divide(o, c)
        cdef np.ndarray[np.float64_t, ndim=1] log_oc = apply_log(oc)
        cdef np.ndarray[np.float64_t, ndim=1] log_oc_sq = log_oc**2

        # log_cc = (df['close'] / df['close'].shift(1)).apply(np.log)
        cdef np.ndarray[np.float64_t, ndim=1] cc = np.divide(c, np_shift(c, 1))
        cdef np.ndarray[np.float64_t, ndim=1] log_cc = apply_log(cc)
        cdef np.ndarray[np.float64_t, ndim=1] log_cc_sq = log_cc**2

        cdef np.ndarray[np.float64_t, ndim=1] rs = log_ho * (log_ho - log_co) + log_lo * (log_lo - log_co)


        # close_vol = pd.Series(log_cc_sq).rolling(
        #     window=window, center=False).sum() * (1.0 / (window - 1.0))
        close_vol = np.sum(sliding_window_view(log_cc_sq, window)) * (1.0 / (window - 1.0))

        # open_vol = pd.Series(log_oc_sq).rolling(
        #     window=window, center=False).sum() * (1.0 / (window - 1.0))
        open_vol = np.sum(sliding_window_view(log_oc_sq, window)) * (1.0 / (window - 1.0))

        # window_rs = pd.Series(rs).rolling(
        #     window=window, center=False).sum() * (1.0 / (window - 1.0))
        window_rs = np.sum(sliding_window_view(rs, window)) * (1.0 / (window - 1.0))

        k = 0.34 / (1.34 + (window + 1) / (window - 1))
        yz = pd.Series(open_vol + k * close_vol + (1 - k) * window_rs).apply(np.sqrt)
        # cdef np.ndarray[np.float64_t, ndim=1] yz = apply_sqrt(open_vol + k * close_vol + (1 - k) * window_rs)
        df['YangZhang_{}_{}'.format(window, p)] = yz
            
        yz_an = pd.Series(open_vol + k * close_vol + (1 - k) * window_rs).apply(np.sqrt) * np.sqrt(af)
        # cdef np.ndarray[np.float64_t, ndim=1] yz_an = apply_sqrt(open_vol + k * close_vol + (1 - k) * window_rs) * np.sqrt(af)
        df['YangZhang{}_{}_Ann'.format(window, p)] = yz_an
    ...
    cpdef double get_arithmetic_sharpe(self, returns, rfr=0, ann_factor=252):
    ...
    cpdef double get_geometric_sharpe(self, returns, rfr=0, ann_factor=252):
        adj_returns = returns - rfr
        return np.multiply(
            np.divide(
    ...
    cpdef np.ndarray[double] adj_returns(self, returns, adjustment_factor):
        if isinstance(adjustment_factor, (float, int)) and adjustment_factor == 0:
            return returns.values
        return (returns - adjustment_factor).values

    cpdef double downside_risk(self, returns, rfr=0, ann_factor=252):
    ...
    cpdef double get_sortino(self, returns, required_return=0, rfr=0, ann_factor=252):
    ...
    cpdef double get_calmar(self, returns, ann_factor=252, annlzd_ret=None, max_dd=None, cum_rets=None):
        """
    ...
    cpdef double get_omega(self, returns, rfr=0.0, req_return=0.0,
                  ann_factor=252):
    ...
    cpdef double stability_of_series(self, returns):
    ...
    cpdef double get_tail_ratio(self, returns):
    ...
    cpdef double get_VaR(self, returns, period=None, sigma=2.0):
        """
    ...
    cpdef double get_CVaR(self, returns, cutoff=0.05):
        """
    ...
    cpdef dict add_data_summary(self):
    ...
    cpdef dict stats(self, str returns='Price_Returns', str price='close', unsigned long trading_periods=252, unsigned long market_hours=24, str interval='1d', unsigned long vol_window=30, bint geo=True):
        """
    ...
    def get_fibs(self, start=None, end=None, period=None, utc=True):
        ...
        while True:
            try:
                ...
                self.f_382 = np.round(
                    self.high-((self.high-self.low)*(1-(1/phi))), 2)
                self.f_50 = np.round(self.high-((self.high-self.low)*0.5), 2)
                self.f_618 = np.round(
                    self.high-((self.high-self.low)*(1/phi)), 2)
